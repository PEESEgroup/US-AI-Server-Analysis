...
frozen_data=np.loadtxt(r'D:\2023 Fall\Github Data\state_REcon_CUE_last25.txt',delimiter=' ',dtype='float')  
...
# Caculation process
with open(r'D:\2023 Fall\spatial scenario\PUE\Low_PUE.csv', newline='') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
...
for i in range (tem_sce_num):
    for j in range (spt_sce_num):
        for k in range (typ_sce_num):
            ...
            name=r'D:\2023 Fall\Github Data\cases_REcon_'+repr(i+1)+'_CUE_last25_CF.txt'
            e_data=np.loadtxt(name,delimiter=' ',dtype='float')
            name=r'D:\2023 Fall\Github Data\cases_REcon_'+repr(i+1)+'_CUE_last25_WF.txt'
