...
frozen_data=np.loadtxt(r'D:\2023 Fall\R2 Files\Inputs\Spatial Dirstribution\R2_spatial.txt',delimiter='\t',dtype='float')  
...
for i in range (tem_sce_num):
    for j in range (spt_sce_num):
        for k in range (typ_sce_num):
            ...
            name=r'D:\2023 Fall\R2 Files\Inputs\Grid Factor\cases_std_'+repr(i+1)+'_CF.txt'
            e_data=np.loadtxt(name,delimiter=' ',dtype='float')
            name=r'D:\2023 Fall\R2 Files\Inputs\Grid Factor\cases_std_'+repr(i+1)+'_WF.txt'
            ...
            flag=0
            with open(r'D:\2023 Fall\R2 Files\Inputs\Baseline_PUEWUE.csv', newline='') as csvfile:
