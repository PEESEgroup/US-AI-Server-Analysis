...
frozen_data=np.loadtxt(r'D:\2023 Fall\Github Data\state_REcon_WUE_last25.txt',delimiter=' ',dtype='float')  
...
# Caculation process
with open(r'D:\2023 Fall\spatial scenario\WUE\Low_WUE.csv', newline='') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=',', quotechar='|')
...
for i in range (tem_sce_num):
    for j in range (spt_sce_num):
        for k in range (typ_sce_num):
            ...
            name=r'D:\2023 Fall\Github Data\cases_REcon_'+repr(i+1)+'_WUE_last25_CF.txt'
            e_data=np.loadtxt(name,delimiter=' ',dtype='float')
            name=r'D:\2023 Fall\Github Data\cases_REcon_'+repr(i+1)+'_WUE_last25_WF.txt'
                    ...
                    # chosse either airside or waterside economizer
                        ...
                        for i1 in range (L_1):
                            ...
                            if rand_num> DLC_rate[i1] :
                                if float(row[8]) > float(row[9]):
                                    n_count=4
                                else:
                                    n_count=5
                            else:
                                if float(row[10]) > float(row[11]):
                                    n_count=6
                                else:
                                    n_count=7  
